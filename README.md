...
| v1 (폐기) | v2 | v3 |
|---|---|---|
| Godot 4 + Vercel Web export | Godot 4 + macOS .app 전용 | + 전투 화면 (BattleScene + 3상태머신) |
| KayKit 3D GLB (5 heroes + 4 skeletons + ~50 props) | Wesnoth 2D PNG (5 factions, 432 자산) | 동일 |
| lazy fetch (jsDelivr CDN) | res:// 직접 | 동일 |
| 검은 화면 (WebGL2 + COEP 헤더 함정) | 1280×720 매너 대시보드 즉시 표시 | + 타이틀 화면 + 호버 인터랙션 + 시간대 그라디언트 |
| 12차까지 누적된 v1 코드 | 클린 리셋 (단 1 commit = v2.0.0) | v2.0.0 → v2.3.5 → v3.0.0 (전투), v3.0.3 (CanvasLayer) |
| 결정 큐 알림만 (LOW/MEDIUM 안 보임) | 모든 우선순위 모달 + 자동 진행 분기 | + BANDIT_RAID HIGH 추가 (12h마다) |
| (해당 없음) | GameWorld.apply_result (결과 코드 → 자원 변동) | BATTLE_* 전용 코드 통합 |
